...
class PostItemMapper @Inject constructor() {
    private fun mapToPresentation(combined: CombinedUserPost): PostItem =
    ...
    fun mapToPresentation(combined: List<CombinedUserPost>): List<PostItem> = combined.map { mapToPresentation(it) }
}